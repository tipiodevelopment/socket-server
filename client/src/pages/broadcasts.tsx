...
function CampaignAppLabel({ campaignName, clientAppName }: { campaignName: string | null; clientAppName: string | null }) {
  ...
  return (
    <div className="flex items-center space-x-2 text-xs text-gray-500 mb-3">
      {campaignName && <span className="text-gray-300">{campaignName}</span>}
      {campaignName && clientAppName && <span className="text-gray-700">/</span>}
      {clientAppName && <span className="text-gray-300">{clientAppName}</span>}
    </div>
  );
}
      ...
      >
        <div className="flex justify-between items-start mb-4">
          <div className="flex-1">
            <div className="flex items-center space-x-3 mb-2">
              <h3 className="text-base font-semibold text-white" data-testid={`text-broadcast-name-${broadcast.broadcastId}`}>{broadcast.broadcastName}</h3>
              <span className="px-2 py-0.5 bg-white text-black text-[10px] uppercase font-bold rounded-full" data-testid={`badge-status-${broadcast.broadcastId}`}>Live</span>
              {broadcast.startTime && (
            ...
            {broadcast.description && (
              <div className="text-sm text-gray-400 mb-3" data-testid={`text-broadcast-desc-${broadcast.broadcastId}`}>{broadcast.description}</div>
            )}
            <CampaignAppLabel campaignName={broadcast.campaignName} clientAppName={broadcast.clientAppName} />
          </div>
          <span className="px-4 py-1.5 bg-white hover:bg-gray-200 text-black rounded text-xs font-medium" data-testid={`button-manage-broadcast-${broadcast.broadcastId}`}>
            Manage
          </span>
        </div>
        <div className="grid grid-cols-5 gap-4 pt-4 border-t border-white/5">
          <div>
            ...
            <div className="text-lg font-semibold text-white" data-testid={`stat-viewers-${broadcast.broadcastId}`}>{viewers > 0 ? formatViewers(viewers) : '—'}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 mb-1">Active Polls</div>
            <div className="text-lg font-semibold text-white" data-testid={`stat-active-polls-${broadcast.broadcastId}`}>{broadcast.activePollCount}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 mb-1">Total Polls</div>
            <div className="text-lg font-semibold text-white" data-testid={`stat-total-polls-${broadcast.broadcastId}`}>{broadcast.pollCount}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 mb-1">Contests</div>
            <div className="text-lg font-semibold text-white" data-testid={`stat-contests-${broadcast.broadcastId}`}>{broadcast.contestCount}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 mb-1">Engagement</div>
            <div className="text-lg font-semibold text-white" data-testid={`stat-engagement-${broadcast.broadcastId}`}>{engagement > 0 ? `${engagement}%` : '—'}</div>
          </div>
        </div>
      </div>
    </Link>
      ...
      >
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <div className="flex items-center space-x-3 mb-2">
              <h3 className="text-base font-semibold text-white" data-testid={`text-broadcast-name-${broadcast.broadcastId}`}>{broadcast.broadcastName}</h3>
              <span className="px-2 py-0.5 bg-white/10 text-gray-300 text-[10px] uppercase font-bold rounded-full border border-white/20" data-testid={`badge-status-${broadcast.broadcastId}`}>Upcoming</span>
            </div>
            {broadcast.description && (
              <div className="text-sm text-gray-400 mb-3" data-testid={`text-broadcast-desc-${broadcast.broadcastId}`}>{broadcast.description}</div>
            )}
          ...
          <span className="px-4 py-1.5 bg-transparent border border-white/20 hover:border-white/40 text-white rounded text-xs font-medium" data-testid={`button-configure-broadcast-${broadcast.broadcastId}`}>
            Configure
          </span>
        </div>
      </div>
...
function EndedBroadcastCard({ broadcast }: { broadcast: EnrichedBroadcast }) {
  ...
  return (
    <Link href={`/broadcasts/${broadcast.broadcastId}`}>
      <div
        className="bg-transparent border border-white/10 rounded-lg p-5 hover:border-white/30 transition-all cursor-pointer opacity-60"
        data-testid={`card-broadcast-${broadcast.broadcastId}`}
      >
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <div className="flex items-center space-x-3 mb-2">
              <h3 className="text-base font-semibold text-white" data-testid={`text-broadcast-name-${broadcast.broadcastId}`}>{broadcast.broadcastName}</h3>
              <span className="px-2 py-0.5 bg-white/10 text-gray-400 text-[10px] uppercase font-bold rounded-full border border-white/10" data-testid={`badge-status-${broadcast.broadcastId}`}>Ended</span>
            </div>
            {broadcast.description && (
              <div className="text-sm text-gray-400 mb-3" data-testid={`text-broadcast-desc-${broadcast.broadcastId}`}>{broadcast.description}</div>
            )}
          ...
          <span className="px-4 py-1.5 bg-transparent border border-white/20 hover:border-white/40 text-white rounded text-xs font-medium" data-testid={`button-view-broadcast-${broadcast.broadcastId}`}>
            View Report
          </span>
        </div>
      </div>
      ...
      actions={
        <Dialog open={createOpen} onOpenChange={setCreateOpen}>
          <DialogTrigger asChild>
            <Button data-testid="button-create-broadcast" className="gap-2 bg-white hover:bg-gray-200 text-black font-medium" disabled={campaigns.length === 0}>
    ...
    >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-medium transition ${
                statusFilter === opt.value
                  ? 'bg-white text-black'
                  : 'bg-transparent border border-white/20 hover:border-white/40 text-gray-300'
              }`}
        ...
        <div className="flex items-center space-x-3">
          <div className="relative">
            <input
              ...
              className="w-64 px-3 py-1.5 bg-transparent border border-white/20 rounded text-sm text-white placeholder-gray-500 focus:outline-none focus:border-white/40"
              value={searchQuery}
          ...
          <button className="w-8 h-8 flex items-center justify-center rounded border border-white/20 hover:border-white/40 text-gray-400 hover:text-white transition" data-testid="button-filter">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16">
          <Radio className="w-12 h-12 text-gray-500 mb-4" />
          <h3 className="text-lg font-semibold mb-2 text-white">No broadcasts found</h3>
          ...
          {campaigns.length > 0 && !searchQuery && statusFilter === 'all' && (
            <Button onClick={() => setCreateOpen(true)} className="bg-white hover:bg-gray-200 text-black" data-testid="button-create-first-broadcast">
        ...
        <div>
          {(statusFilter === 'all' || statusFilter === 'live') && liveBroadcasts.length > 0 && (
            <div className="mb-8" data-testid="section-live-broadcasts">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-semibold text-gray-400 uppercase">Live Now</h2>
                <span className="text-xs text-gray-500">{liveBroadcasts.length} broadcast{liveBroadcasts.length !== 1 ? 's' : ''}</span>
              </div>
          ...
          {(statusFilter === 'all' || statusFilter === 'upcoming') && upcomingBroadcasts.length > 0 && (
            <div className="mb-8" data-testid="section-upcoming-broadcasts">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-semibold text-gray-400 uppercase">Upcoming</h2>
                <span className="text-xs text-gray-500">{upcomingBroadcasts.length} broadcast{upcomingBroadcasts.length !== 1 ? 's' : ''}</span>
              </div>
              <div className="space-y-3">
          ...
          {(statusFilter === 'all' || statusFilter === 'ended') && endedBroadcasts.length > 0 && (
            <div className="mb-8" data-testid="section-ended-broadcasts">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-semibold text-gray-400 uppercase">Recently Ended</h2>
                <span className="text-xs text-gray-500">{endedBroadcasts.length} broadcast{endedBroadcasts.length !== 1 ? 's' : ''}</span>
              </div>
              <div className="space-y-3">
