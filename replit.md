...
- **Campaign Settings tab reorganized:** Removed obsolete sections: Campaign Logo (branding comes from Sponsor), Match Context (covered by broadcast externalId), and UI Theme (colors belong to Sponsor). Remaining sections: Basic Info, Schedule, Channel Assignment, Tipio Integration, Targeting & Segmentation, Engagement Settings, Feature Flags, Danger Zone. UI restyled to match the monochromatic dark pattern used in OverviewTab — raw divs with `border-white/10` instead of shadcn Cards.
- **Tipio Integration (renamed from Reachu):** "Reachu Integration" renamed to "Tipio Integration" in all UI labels and documentation. DB fields remain `campaigns.reachuApiKey` / `campaigns.reachuChannelId` (internal names preserved). The Tipio API key is now included in `GET /v1/campaigns/:id/config` response as `integrations.tipio.apiKey` — SDK reads it dynamically from the server instead of hardcoding it in the app config. The `integrations.tipio` block is always present in the response (`enabled: false` when no key configured).
- **API Key Architecture (definitive):** The SDK uses ONE single Vio App API Key (`client_apps.api_key`) for all Vio backend endpoints. Tipio module key is campaign-level, delivered via config endpoint. No `campaignAdminApiKey` or `campaignApiKey` fields needed in `vio-config.json`.
