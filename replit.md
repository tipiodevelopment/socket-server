...
- **Tipio Integration (renamed from Reachu):** "Reachu Integration" renamed to "Tipio Integration" in all UI labels and documentation. DB fields remain `campaigns.reachuApiKey` / `campaigns.reachuChannelId` (internal names preserved). The Tipio API key is now included in both `GET /v1/campaigns/:id/config` and `GET /v1/sdk/config` responses as `integrations.commerce.apiKey` — SDK reads it dynamically from the server. The `integrations.commerce` block is always present in both responses (`enabled: false` when no key configured). "Tipio" is the definitive name — no further renames planned.
- **API Key Architecture (definitive):** The SDK uses ONE single Vio App API Key (`client_apps.api_key`) for all Vio backend endpoints. Tipio module key is campaign-level, delivered via config endpoint. No `campaignAdminApiKey` or `campaignApiKey` fields needed in `vio-config.json`. Legacy `vio-config.json` with three keys still works but is deprecated.
