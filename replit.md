...
- **Tipio Integration (renamed from Reachu):** "Reachu Integration" renamed to "Tipio Integration" in all UI labels and documentation. DB fields remain `campaigns.reachuApiKey` / `campaigns.reachuChannelId` (internal names preserved). The Tipio API key is now included in both `GET /v1/campaigns/:id/config` and `GET /v1/sdk/config` responses as `integrations.tipio.apiKey` — SDK reads it dynamically from the server. The `integrations.tipio` block is always present in both responses (`enabled: false` when no key configured). "Tipio" is the definitive name — no further renames planned.
