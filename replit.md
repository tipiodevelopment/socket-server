...
- **Channel moved to campaign level:** `channels.client_app_id` is now nullable — channels are standalone entities, no longer owned by apps. SDK discovery (`GET /v1/sdk/campaigns`) now resolves campaigns directly via `campaigns.client_app_id` instead of navigating app → channels → campaigns. `GET /v1/sdk/config` and `GET /v1/campaigns/:id/config` now authorize via `campaign.clientAppId` with fallback to legacy channel auth. Channel assignment in SettingsTab remains optional metadata. Existing channel-linked campaigns continue to work (backwards compatible).
