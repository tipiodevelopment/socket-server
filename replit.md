...
- **Commerce Integration (renamed from Reachu):** "Reachu Integration" renamed to "Commerce Integration" in all UI labels and documentation. DB fields remain `campaigns.reachuApiKey` / `campaigns.reachuChannelId` (internal names preserved). The Commerce API key is now included in `GET /v1/campaigns/:id/config` response as `integrations.commerce.apiKey` — SDK reads it dynamically from the server instead of hardcoding it in the app config. The `integrations.commerce` block is always present in the response (`enabled: false` when no key configured).
- **API Key Architecture (definitive):** The SDK uses ONE single Vio App API Key (`client_apps.api_key`) for all Vio backend endpoints. Commerce module key is campaign-level, delivered via config endpoint. No `campaignAdminApiKey` or `campaignApiKey` fields needed in `vio-config.json`.
