...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  wss.on("connection", (ws: WebSocket, request: any, campaignId: number) => {
    ...
    console.log(
      `Client connected to campaign ${campaignId}. Total clients: ${storage.getConnectedClientsCount()}`
    );
    ...
    ws.on("close", () => {
      ...
      console.log(
        `Client disconnected from campaign ${campaignId}. Total clients: ${storage.getConnectedClientsCount()}`
      );
      broadcastClientCountToCampaign(campaignId);
    });
