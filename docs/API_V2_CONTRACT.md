...
>
> **Status**: Draft 2026-04-24. Target: approved + adopted before Kotlin SDK
> coding starts and before the next iOS SDK release.
...
| GET | `/v2/mobile/broadcasts/:broadcastId/score` | Live score snapshot. **Replaces `/v1/sdk/broadcasts/:id/score`.** |
| GET | `/v2/mobile/broadcasts/:broadcastId/stats` | Match stats snapshot. **Replaces `/v1/sdk/broadcasts/:id/stats`.** |

Full shapes → `openapi.yaml` tagged `mobile-sdk`.

### Engagement under mobile

Polls + contests stay as-is semantically but move under `/v2/mobile/engagement/*`:

| Method | Path | Purpose |
|---|---|---|
| GET | `/v2/mobile/engagement/broadcasts/:broadcastId/polls` | Active polls list |
| POST | `/v2/mobile/engagement/polls/:pollId/vote` | Cast a vote (rate-limited 30/min) |
| GET | `/v2/mobile/engagement/broadcasts/:broadcastId/contests` | Active contests list |
| POST | `/v2/mobile/engagement/contests/:contestId/participate` | Submit contest entry (rate-limited 10/min) |
...
Dashboard-operator paths (`/api/broadcasts/:id/trigger-shoppable-ad`,
`/api/broadcasts/:id/sponsor-slots/:slotId/execute`, slot CRUD, sponsor CRUD,
campaign CRUD, placement CRUD, analytics) remain under `/api/*` — they are
session-auth dashboard routes, not part of the SDK contract.
...
- `410` — **Gone**. Used exclusively for v1 routes post-cutover. Response
  body includes `Link: </v2/mobile/...>; rel="successor-version"`.
...
- **v1 retirement** → announce date in release notes, serve `410 Gone` with
  `Link` header from that date. No parallel window.
...
|---|---|---|
| `GET /v1/sdk/config` | `GET /v2/mobile/config` | iOS, Kotlin mobile |
| `GET /v2/sdk/config` | `GET /v2/mobile/config` | iOS (already on v2-ish) |
...
| `GET /v1/sdk/broadcasts/:id/lineup` | `GET /v2/mobile/broadcasts/:id/lineup` | iOS |
| `GET /v1/sdk/broadcasts/:id/chat` | `GET /v2/mobile/broadcasts/:id/chat` | iOS |
| `GET /v1/sdk/broadcasts/:id/score` | `GET /v2/mobile/broadcasts/:id/score` | iOS |
| `GET /v1/sdk/broadcasts/:id/stats` | `GET /v2/mobile/broadcasts/:id/stats` | iOS |
| `GET /v1/engagement/polls` | `GET /v2/mobile/engagement/broadcasts/:id/polls` | iOS |
| `POST /v1/engagement/polls/:id/vote` | `POST /v2/mobile/engagement/polls/:id/vote` | iOS |
| `GET /v1/engagement/contests` | `GET /v2/mobile/engagement/broadcasts/:id/contests` | iOS |
| `POST /v1/engagement/contests/:id/participate` | `POST /v2/mobile/engagement/contests/:id/participate` | iOS |
| `POST /api/sdk/tv/broadcast/subscribe` | `POST /v2/tv/broadcast/subscribe` | VioTVSDK, Kotlin TV |
| `POST /api/sdk/tv/session/*` | `POST /v2/tv/session/*` | VioTVSDK, Kotlin TV |
| `POST /api/sdk/tv/cart-intent` | `POST /v2/tv/cart-intent` | VioTVSDK, Kotlin TV |
| `POST /api/sdk/tv/broadcasts/:id/shoppable-ad` | `POST /v2/tv/broadcasts/:id/shoppable-ad` | VioTVSDK, Kotlin TV |
...
**Unchanged**: everything under `/api/*` that is session-cookie auth
(dashboard CRUD for campaigns, sponsors, slots, broadcasts, analytics,
uploads, form-state, etc.). Those are not SDK-facing.

**Retired** (will 410 at cutover): `/v1/sdk/campaigns`, `/v1/sdk/broadcast`,
`/v1/sdk/components`, `/v1/sdk/livescores`, `/v1/campaigns/:id/config`,
`/v1/engagement/config`, `/v1/localization/:lang`, `/v1/offers` — these
predate the multi-sponsor model and have no v2 equivalent planned. Flag each
for "do any SDKs still call this?" audit before the cutover date.
